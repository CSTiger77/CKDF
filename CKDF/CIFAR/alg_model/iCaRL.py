...
from public.util_models import FeatureExtractor, FE_cls, FE_2fc_cls, FE_3fc_cls
...
# ----- iCaRL -----#
# --------------------#
from public.utils import AverageMeter, accuracy, strore_features, FC_strore_features, split_array
...
class iCaRL(ExemplarHandler):
    ...
    def construct_model(self, rate):
        if self.availabel_cudas:
            ...
            model = torch.nn.DataParallel(FE_cls(resnetforcifar.__dict__[self.model_name](rate=rate, get_feature=True),
                                                 self.hidden_size, self.num_classes), device_ids=device_ids).cuda()
        else:
            ...
            if task > 1:
                ...
                for epoch in range(1, self.epochs + 1):
                    ...
                    while iters_left > 0:
                        ...
                        iter_index += 1
                        if iter_index % args.print_interval == 0:
                            rlts = f"train: epoch {epoch:0>3d}, iter [{iter_index:0>4d}, {iter_num:0>4d}], " \
                                   f"lr: {scheduler.get_last_lr()[0]:.6f}, top1 acc: {loss_dict['top1']:.2f}%, top5 acc: " \
                                   f"{loss_dict['top5']:.2f}%, loss_total: {loss_dict['losses']:.2f}"
                            self.batch_train_logger.info(
                                rlts
                ...
                if task == 1:
                    ...
                    # strore_features(self.FE_cls, self.train_datasets[0], self.classes_per_task, 0,
                    #                 args.checkpoints)
                    # strore_features(self.FE_cls, self.train_datasets[1], self.classes_per_task, 1,
                    #                 args.checkpoints)
            else:
                if self.rate == 1:
                    if self.lr == 1:
                        ...
                        model_path_for_task2 = "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/" \
                                               "cifar10pretrain/_Test_iCaRL_original/Test_unbias/task1/rate_/test1/" \
                        ...
                        model_path_for_task50 = "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/" \
                                                "cifar10pretrain/_Test_iCaRL_original/visualization/task50/rate_/test3/" \
                        ...
                        #                        "cifar100_checkpoints/50_cifar100_resnet34_iCaRL_rate_1_FE_cls_task1.pth"
                        model_path_for_task25 = "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/" \
                                                "cifar10pretrain/_Test_iCaRL_original/visualization/task25/rate_/test3/" \
                                                "cifar100_checkpoints/4_cifar100_resnet34_iCaRL_rate_1_FE_cls_task1.pth"
                        ...
                        model_path_for_task50 = "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/" \
                                                ...
                                                "cifar100_checkpoints/2_cifar100_resnet34_iCaRL_rate_1_FE_cls_task1.pth"
                        # model_path_for_task50 = "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/" \
                        #                         "cifar10pretrain/_Test_iCaRL_original/visualization/task25/rate_/" \
                        #                         "test3/cifar100_checkpoints/4_cifar100_resnet34_iCaRL_rate_1_FE_cls_task1.pth"
                    elif self.lr == 2:
                        # model_path_for_task2 = "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_July/" \
                        #                        "cifar10pretrain/_Test_iCaRL_original/iCaRL_cmp/task2/test1/rate_/" \
                        #                        "cifar100_checkpoints/50_cifar100_resnet34_iCaRL_rate_1_FE_cls_task1.pth"
                        model_path_for_task2 = "/share/home/kcli/CL_research/iCaRL_ILtFA/checkpoints/" \
                                               "cifar100_resnet34_rate_1_FE_cls.pth"
                        model_path_for_task5 = "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_July/" \
                ...
                if self.tasks == 2:
                    ...
                    model_path = model_path_for_task25
                elif self.tasks == 50:
                    model_path = model_path_for_task50

                self.FE_cls = self.load_FE_cls_model(model_path)
                # strore_features(self.FE_cls, self.val_datasets[0], self.classes_per_task, 0,
                #                 args.checkpoints)
                ...
                for epoch in range(1, self.epochs + 1):
                    ...
                    while iters_left > 0:
                        ...
                        try:
                            ...
                            raise ValueError("next(data_loader) error while read data. ")
                        x, y = x.to(self.device), y.to(self.device)  # --> transfer them to correct device
                        if task > 1 and self.pre_FE_cls is not None:
    ...
    def feature_relearning_train_main_retrain_cls_of_FMcls(self, args):
        '''Train a model (with a "train_a_batch" method) on multiple tasks, with replay-strategy specified by [replay_mode].
        ...
        for task, train_dataset in enumerate(self.train_datasets, 1):
            ...
            if task > 1 and self.memory_budget > 0:
                ...
                feature_relearn_exemplar_dataset = examplar_train_dataset
                ...
                for class_index in range(begin_class, end_class_plus_1):
                    ...
                    train_dataset_temp, val_dataset_temp = torch.utils.data.random_split(
                        ...
                        generator=torch.Generator().manual_seed(
                            0))
                    if per_task_training_dataset is None:
                        per_task_training_dataset = train_dataset_temp
                    else:
            ...
            if task > 1:
                ...
                if task > 1:
                    self.logger.info(f'New task {task} begin: use feature_relearn_exemplar_dataset '
                                     ...
                                     f'use feature_relearn_exemplar_dataset to train FE_cls.')
                    assert feature_relearn_exemplar_dataset is not None
                    self.EFAfIL_split_feature_mapper_cls_domain_train(per_task_training_dataset,
                    ...
                    if args.train_bias_cls_of_FMcls:
                        self.retrain_bias_cls_of_FMcls(per_task_valing_dataset, self.val_datasets, task,
                    ...
                    while iters_left > 0:
                        ...
                        if task > 1 and self.pre_FE_cls is not None:
                            ...
                            with torch.no_grad():
                                ...
                                features = self.pre_FE_cls(x)[-2]
                                FM_features, scores, _ = self.FM_cls_domain(features)
                        ...
                        if args.train_bias_cls_of_FMcls:
                            ...
                            with torch.no_grad():
                                if args.bias_or_cRT == 0:
                                    ...
                                    scores = self.unbias_cls_of_FMcls(scores, self.classes_per_task)
                                else:
                                    scores = self.unbias_cls_of_FMcls(FM_features)
                                    if active_classes is not None:
                                        class_entries = active_classes[-1] if type(
                                            active_classes[0]) == list else active_classes
                                        scores = scores[:, class_entries]
    ...
    def feature_relearning_train_main_for_visualization(self, args):
        '''Train a model (with a "train_a_batch" method) on multiple tasks, with replay-strategy specified by [replay_mode].
        ...
        for task, train_dataset in enumerate(self.train_datasets, 1):
            ...
            if task > 2:
                ...
                if task == 1:
                    checkpoint_file = os.path.join(args.checkpoints,
                                                   "{}_cifar100_resnet34_iCaRL_rate_{}_FE_cls_task_{}.pth".format(
                                                       self.classes_per_task, self.rate, task))
                    torch.save(self.FE_cls.module.state_dict(), checkpoint_file)
                elif task <= 2:
                    checkpoint_file = os.path.join(args.checkpoints,
                                                   "{}_cifar100_resnet34_iCaRL_rate_{}_FE_cls_task_{}.pth".format(
                                                       self.classes_per_task, self.rate, task))
                    torch.save(self.FE_cls.module.state_dict(), checkpoint_file)
                    FM_cls_domain_checkpoint_file = os.path.join(args.checkpoints,
                                                                 "FC_task_{}.pth".format(
                                                                     self.classes_per_task, self.rate, task))
                    torch.save(self.FM_cls_domain.module.state_dict(), FM_cls_domain_checkpoint_file)
                else:
                    break
                if task == 2:
                    FC_strore_features(self.pre_FE_cls, self.FM_cls_domain, self.train_datasets[0],
                                       self.classes_per_task,
                                       0,
                                       args.checkpoints)
                    FC_strore_features(self.pre_FE_cls, self.FM_cls_domain, self.train_datasets[1],
                ...
                #         "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/cifar10pretrain/"
                #         "_Test_iCaRL_original/visualization/FC_iCaRL/task50/MLP3/rate_/test2/cifar100_checkpoints/"
                #         "2_cifar100_resnet34_iCaRL_rate_1_FE_cls_task_1.pth")
                # elif task == 2:
                #     self.FE_cls = self.load_FE_cls_model(
                #         "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/cifar10pretrain/"
                #         "_Test_iCaRL_original/visualization/FC_iCaRL/task50/MLP3/rate_/test2/cifar100_checkpoints/"
                #         "2_cifar100_resnet34_iCaRL_rate_1_FE_cls_task_2.pth")
                if task == 1:
                    self.FE_cls = self.load_FE_cls_model(
                        "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/cifar10pretrain/"
                        "_Test_iCaRL_original/visualization/task25/rate_/test3/cifar100_checkpoints/"
                        "4_cifar100_resnet34_iCaRL_rate_1_FE_cls_task1.pth")
                elif task == 2:
                    self.FE_cls = self.load_FE_cls_model(
                        "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/cifar10pretrain/"
                        "_Test_iCaRL_original/visualization/task25/rate_/test3/cifar100_checkpoints/"
    ...
    def train_a_batch(self, x, y, scores, active_classes, optimizer, task=1):
        ...
        if x is not None:
            ...
            #     y_hat_for_distill /= self.KD_temp
            #     y_hat_for_cls = y_hat[:, -self.classes_per_task:]
            ...
            #         input=y_hat_for_cls, target=binary_targets, reduction='none'
            #     ).sum(dim=1).mean()  # --> sum over classes, then average over batch
            # predL = loss_cls + loss_distill * (self.KD_temp ** 2)
            if len(active_classes) >= 5:
                acc1, acc5 = accuracy(y_hat, y, topk=(1, 5))
                top1.update(acc1.item(), x.size(0))
                top5.update(acc5.item(), x.size(0))
    ...
    def feature_relearning_train_main_alldtl(self, args):
        '''Train a model (with a "train_a_batch" method) on multiple tasks, with replay-strategy specified by [replay_mode].
        ...
        for task, train_dataset in enumerate(self.train_datasets, 1):
            ...
            if task >= 1:
                active_classes = list(range(self.classes_per_task * task))
                if task > 1:
                    self.logger.info(f'New task {task} begin: use exemplar_dataset to train FA'
                                     f'use exemplar_dataset to train FE_cls.')
                    assert feature_relearn_exemplar_dataset is not None
                    self.EFAfIL_split_feature_mapper_cls_domain_train(train_dataset, feature_relearn_exemplar_dataset,
                                                                      self.val_datasets,
                ...
                for epoch in range(1, self.epochs + 1):
                    ...
                    while iters_left > 0:
                        ...
                        # ---> Train MAIN MODEL
                        # Train the main model with this batch
                        loss_dict = self.train_a_batch_all_dtl(x, y, scores, active_classes, optimizer, task)
                        iter_index += 1
                        if iter_index % args.print_interval == 0:
                            self.batch_train_logger.info(
                                f"train: epoch {epoch:0>3d}, iter [{iter_index:0>4d}, {iter_num:0>4d}], "
    ...
    def train_a_batch_all_dtl(self, x, y, scores, active_classes, optimizer, task=1):
        '''Train model for one batch ([x],[y]), possibly supplemented with replayed data ([x_],[y_/scores_]).
        ...
        if x is not None:
            ...
            if scores is not None:
                ...
                loss_distill = Func.binary_cross_entropy_with_logits(
                    input=y_hat_for_distill, target=binary_targets, reduction='none'
                ).sum(dim=1).mean()
    ...
    def extraData_train_main(self, args):
        '''Train a model (with a "train_a_batch" method) on multiple tasks, with replay-strategy specified by [replay_mode].
        ...
        for task, train_dataset in enumerate(self.train_datasets, 1):
            ...
            if task >= 1:
                ...
                for epoch in range(1, self.epochs + 1):
                    ...
                    while iters_left > 0:
                        ...
                        if self.pre_FE_cls is not None:
                            with torch.no_grad():
                                scores = self.pre_FE_cls(x)[-1][:, :(self.classes_per_task * (task - 1))]
                                extra_scores = self.pre_FE_cls(extra_x)[-1][:, :(self.classes_per_task * (task - 1))]
                        else:
                            scores = None
                            extra_scores = None
                        ...
                        # ---> Train MAIN MODEL
                        # Train the main model with this batch
                        loss_dict = self.train_a_batch_plus_extraData(x, y, extra_x, extra_y, scores, extra_scores,
                                                                      active_classes,
                                                                      optimizer, task)
                        iter_index += 1
                        if iter_index % args.print_interval == 0:
                ...
                # if task == 1:
                #     torch.save(self.FE_cls, "featureExtractor_task_1.pth")
            # else:
            #     # self.FE_cls = torch.load("")
            #     self.FE_cls = torch.load("cifar10_preTrain_task_1_FE_cls.pth")
            #     # continue
    ...
    def test_upper_bound(self, args):
        # model_path_for_task2 = "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/" \
        #                        "cifar10_Test/LwF_EXP/Test_unbias/task1/rate_/test_1/cifar100_checkpoints/" \
        #                        "10_cifar100_resnet34_LwF_rate_1_FE_cls_task1.pth"
        model_path = "/share/home/kcli/CL_research/iCaRL_ILtFA/cifar100EXP_resnet34_Aug/cifar10pretrain/" \
                     "_Test_iCaRL_original/visualization/task25/rate_/test3/cifar100_checkpoints/" \
                     "4_cifar100_resnet34_iCaRL_rate_1_FE_cls_task1.pth"
        self.FE_cls = self.load_FE_cls_model(model_path)
        acc_past_tasks, acc_list = self.ncm_tasks_validate(1, classifier="fc")
        predict_result = self.ncm_task_validate(self.val_datasets[0], 0, classifier="fc")
        predict_result = self.ncm_task_validate(self.val_datasets[1], 0, classifier="fc")
        # iCaRL_result["task_{}_fc_results".format(task)] = acc_past_tasks
        print("task: %d fc acc:" % self.tasks, acc_past_tasks)
