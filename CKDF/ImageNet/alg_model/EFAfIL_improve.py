...
from EFAfIL_improve_exemplars import EFAfIL_improve_FeaturesHandler
...
class EFAfIL_improve(EFAfIL_improve_FeaturesHandler):
    def __init__(self, model_name, MLP_name, dataset_name,
        ...
        EFAfIL_improve_FeaturesHandler.__init__(self, MLP_name, num_classes, hidden_size, Exemple_memory_budget,
                                                Feature_memory_budget, norm_exemplars, herding, batch_size, sim_alpha,
                                                MLP_lr,
                                                ...
                                                batch_train_logger, CNN_lr, CNN_momentum, CNN_weight_decay, CNN_milestones,
                                                lrgamma, epochs, optim_type)
    ...
    def copy_from_EFAfIL_model(self):
        if self.FE_cls:
            model = copy.deepcopy(self.FE_cls)
        else:
            model = None
        return model
    ...
    def imgs_2_featureHalder_process(self, x):
        FH_FEcls_features, _ = self.featureHandler_FEcls_result(x)
        self.FM_cls_domain.eval()
        with torch.no_grad():
            FM_features, FM_targets, domain = self.FM_cls_domain(FH_FEcls_features)
        return FM_features, FM_targets
    ...
    def train_main(self, args):
        ...
        for task, train_dataset in enumerate(self.train_datasets, 1):
            ...
            if task > 1:
                self.EFAfIL_feature_mapper_cls_domain_train(training_dataset, self.val_datasets, self.classes_per_task,
                                                            active_classes, task)
                # self.EFAfIL_splitfeature_mapper_cls_domain_train(train_dataset, self.val_datasets,
                ...
                for epoch in range(1, self.epochs + 1):
                    ...
                    while iters_left > 0:
                        ...
                        x, y = next(data_loader)  # --> sample training data of current task
                        x, y = x.to(self.device), y.to(self.device)  # --> transfer them to correct device
                        FM_features, FM_targets = self.imgs_2_featureHalder_process(x)
                        # ---> Train MAIN MODEL
            ...
            acc_past_tasks, acc_list = self.tasks_validate(task, classifier="FH_fc", active_classes=active_classes)
            EFAfIL_result["task_{}_fc_results".format(task)] = acc_past_tasks
            acc_past_tasks, acc_list = self.tasks_validate(task, classifier="FH_FM_cls", active_classes=active_classes)
            EFAfIL_result["task_{}_fc_results".format(task)] = acc_past_tasks
            acc_past_tasks, acc_list = self.tasks_validate(task, classifier="FE_cls", active_classes=active_classes)
    ...
    def train_a_batch_feature_replay(self, x, y, feature_replay_features,
        ...
        if type(self.FM_cls_domain) is torch.nn.DataParallel:
            feature_replay_y_hat = self.FM_cls_domain.module.cls(feature_replay_features)
        else:
            feature_replay_y_hat = self.FM_cls_domain.cls(feature_replay_features)
        # -if needed, remove predictions for classes not in current task
        if active_classes is not None:
            class_entries = active_classes[-1] if type(active_classes[0]) == list else active_classes
            y_hat = y_hat[:, class_entries]
        ...
        loss_sim = 1 - torch.cosine_similarity(features, FM_features).mean()
        # loss_total = loss_cls_distill + self.fd_gamma * loss_sim
        loss_total = loss_cls_distill + loss_sim + loss_distill_feature_replay
        loss_total.backward()
        # Take optimization-step
        optimizer.step()
        # loss_total = self.kd_lamb * loss_distill + self.fd_gamma * loss_sim + loss_cls
    ...
    def train_a_batch_plus_extraData(self, x, y, extra_x, extra_y, FM_targets, FM_features, extra_FM_features,
        ...
        # loss_total = loss_cls_distill + self.fd_gamma * loss_sim
        loss_total = loss_cls_distill + loss_sim + loss_extra_distill + loss_extra_sim
        loss_total.backward()
        # Take optimization-step
        optimizer.step()
        # loss_total = self.kd_lamb * loss_distill + self.fd_gamma * loss_sim + loss_cls
    ...
    def per_task_validate(self, val_dataset, task, classifier="linearSVM", active_classes=None):
        # todo
