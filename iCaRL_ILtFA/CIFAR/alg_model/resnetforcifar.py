...
class ResNet(nn.Module):
    def __init__(self, block, layers, rate=1, get_feature=False, num_classes=100, inter_layer=False):
...
def _resnet(arch, block, layers, pretrained, progress, rate,  **kwargs):
    model = ResNet(block, layers, rate,  **kwargs)
    return model


def resnet18(pretrained=False, progress=True, rate=1,  **kwargs):
    return _resnet('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress, rate,
                   **kwargs)


def resnet32(pretrained=False, progress=True, rate=1, **kwargs):
    return _resnet('resnet32', BasicBlock, [3, 4, 5, 3], pretrained, progress, rate,
                   **kwargs)


def resnet34(pretrained=False, progress=True, rate=1,  **kwargs):
    return _resnet('resnet34', BasicBlock, [3, 4, 6, 3], pretrained, progress, rate,
                   **kwargs)
