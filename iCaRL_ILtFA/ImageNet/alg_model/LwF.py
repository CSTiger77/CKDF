...
from public.util_models import FeatureExtractor, FE_cls, SoftTarget_CrossEntropy
...
class LwF(ExemplarHandler):
    def __init__(self, model_name, dataset_name, dataset_path, dataset_json_path, num_classes, hidden_size,
                 ...
                 MLP_name=None, MLP_distill_rate=None,
                 MLP_KD_temp=None, MLP_KD_temp_2=None, MLP_lr=None, MLP_rate=None, MLP_momentum=None,
                 MLP_epochs=None, MLP_milestones=None,
                 MLP_weight_decay=None,
                 MLP_lrgamma=None, MLP_optim_type=None, seed=0):
            ...
            if task > 1:
                ...
                for epoch in range(1, self.epochs + 1):
                    ...
                    while iters_left > 0:
                        ...
                        loss_dict = self.train_a_batch_MC(x, y, scores,
    ...
    def LwF_MC_feature_relearning_train_main(self, args):
        ...
        # Loop over all tasks.
        iCaRL_result = {"timestamp": str(datetime.datetime.now()), "model_rate": self.rate, "is_LwF_MC": args.is_LwF_MC,
                        "feature_oversample": args.feature_oversample}
        iCaRL_result.update(self.data_config)
        img_transform = transforms.Compose([
            *AVAILABLE_TRANSFORMS["imagenet_examplar"]["train_transform"],
        ])
        for task, train_dataset in enumerate(self.train_datasets, 1):
            ...
            if task > 1:
                active_classes = list(range(self.classes_per_task * task))
                if task > 1:
                    self.logger.info(f'New task {task} begin: use exemplar_dataset '
                                     f'to train FA'
                    ...
                    #                                                       active_classes, task)
                    # else:
                    #     self.feature_mapper_cls_domain_train(training_dataset,
                    #                                          self.val_datasets,
                    #                                          self.classes_per_task,
                    #                                          active_classes, task)
                    self.pre_FE_cls = copy.deepcopy(self.FE_cls).eval()
